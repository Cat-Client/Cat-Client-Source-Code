...
public class ServerList
{
    ...
    private void loadFeaturedServers() {
    	this.addServerData(new ServerFeaturedData("VoidSmelt", "voidsmelt.apex.mc.co"));
	}
    
    public int getFeaturedServerCount() {
    	int count = 0;
    	for (ServerData sd : this.servers) {
    		if (sd instanceof ServerFeaturedData) {
    			count ++;
    		}
    	}
    	return count;
    ...
    public void saveServerList()
    {
        try
        {
            ...
            {
                if (!(serverdata instanceof ServerFeaturedData)) {
