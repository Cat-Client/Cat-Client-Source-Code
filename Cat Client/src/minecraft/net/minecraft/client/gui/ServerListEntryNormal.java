...
import catclient.servers.ServerFeaturedData;
...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        boolean isFeaturedServer = (server instanceof ServerFeaturedData);
        if (isFeaturedServer) {
        	drawImg(x, y, true, ServerFeaturedData.ICON);
        }
    ...
    private void drawImg(int x, int y, boolean lower, ResourceLocation icon) {
		this.mc.getTextureManager().bindTexture(icon);
		Gui.drawModalRectWithCustomSizedTexture(x - 18, lower ? y + 8 : y, 0.0F, 0.0F, 16, 16, 16, 16);
	}
    ...
    public boolean mousePressed(int slotIndex, int p_148278_2_, int p_148278_3_, int p_148278_4_, int p_148278_5_, int p_148278_6_)
    {
        if (p_148278_5_ <= 32)
        {
            ...
            if (!(this.owner.getServerList().getServerData(slotIndex) instanceof ServerFeaturedData)) {
